...
void inicializa_BNO(void)
{
  if (!bno.begin())
  {
    // Problema a detectar o BNO055... Verificar as conecções
    Serial.print(F("Ooops, BNO055 nao detectado ... Verificar as coneccoes ou o endereco I2C!"));
    while (1);
  }
  envia_LORA("BNO OK!\n");
...
String inicializa_SD(void)
{
  ...
  {
    envia_LORA(F("ERRO - Cartão SD NÃO detetado\n"));
    delay(1000);
  }
  if(!SD.begin(chipSelect))
  {
            
    envia_LORA(F("Incialização Falhada! - A tentar outra vez"));
    inicializa_SD();
    return nome; 
   ...
   if(ficheiro)
   {
     ficheiro.println(F("----------------AURORA 3.0 - Inicio de Dados-----------------\n"));
     ficheiro.close();
   }
   else
   {
    while(1)
    {
    envia_LORA(F("Erro a abrir o ficheiro"));
    }
   }
    
  envia_LORA(F("SD inicializado com Sucesso!\n"));
  delay(1000);
  return nome;
...
void inicializa_altimetro()
{
  ...
  envia_LORA("Altimetro OK\n");
  
}

//Função GuardaSD
//Guarda os dados para o cartão SD
void guardaSD( double*AcXf, double *AcYf, double *AcZf)
{
  File ficheiro;
